...
class d_spectra_sample:
    ...
    def plot(
        ...
        # Plot initial R vector if available
        if self.initial_R is not None:
            x_positions = np.arange(1, len(self.diffusivities) + 1)
            ax.plot(
                x_positions,
                self.initial_R,
                "r*",
    ...
    def plot_autocorrelation(self, max_lag=2000, ax=None):
        """Plot autocorrelation function (ACF) for each dimension up to max_lag.

        The autocorrelation function for MCMC diagnostics is defined as:
            ρ(k) = γ(k) / γ(0)

        where γ(k) is the autocovariance at lag k:
            γ(k) = 1/(n-k) Σ(t=1 to n-k) [(X_t - μ)(X_{t+k} - μ)]

        and γ(0) is the variance:
            γ(0) = 1/n Σ(t=1 to n) (X_t - μ)²

        Parameters
        ----------
        max_lag : int, optional
            Maximum lag to compute autocorrelation for (default: 2000)
        ...
        [2] Brooks, Steve, et al. Handbook of markov chain monte carlo. CRC press, 2011.
        """
        if not ax:
            fig, ax = plt.subplots(figsize=(10, 6))

        sample_array = np.array(self.sample)
        n_dims = len(self.diffusivities)

        for dim in range(n_dims):
            # Get the chain for this dimension
            ...
            # Compute autocovariance for all lags
            acf = np.zeros(max_lag)
            variance = np.sum(chain_centered**2) / n  # γ(0)

            for k in range(max_lag):
                # Compute autocovariance at lag k
                acf[k] = np.sum(chain_centered[k:] * chain_centered[: (n - k)]) / (
                    n - k
                )
            ...
            # Plot with label showing the diffusivity value
            ax.plot(np.arange(max_lag), acf, label=f"D={self.diffusivities[dim]}")
...
if __name__ == "__main__":
    ...
    configs = {}
    base_config_path = os.path.join(os.path.dirname(__file__), "../../configs.yaml")
