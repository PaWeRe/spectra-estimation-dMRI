...
class GibbsSampler:
    ...
    def run_new(
        ...
        # Determine SNR for the sampler (MOVED UP to ensure consistency)
        ...
        # FIX: Use sampler_sigma consistently (was using 1/SNR, should be 1/sqrt(SNR))
        try:
            posterior_mean, posterior_precision = self.model.get_posterior_params(
                sampler_sigma,
                signal,  # Fixed: now uses same sigma as the rest of the code
            )
            use_full_precision = True
            print(f"[DEBUG] get_posterior_params succeeded, use_full_precision=True")
        except Exception as e:
            use_full_precision = False
            print(f"[DEBUG] get_posterior_params failed: {e}, use_full_precision=False")
        ...
        # Debug: Print which method will be used
        print(
            f"[DEBUG] n_dim={n_dim}, sandy_sampler={sandy_sampler}, use_full_precision={use_full_precision}"
        )
        if sandy_sampler:
            print(f"[DEBUG] Using: sandy_sampler method")
        else:
            print(f"[DEBUG] Using: _gibbs_step_elementwise method")

        for it in iterator:
            # Choose sampling method
            # NOTE: Precision matrix method (_gibbs_step) was found to be buggy.
            # Always use elementwise method which works correctly.
            if sandy_sampler:  # Use original sampling technique
                R_new = R.copy()
                for i in range(N):
                    R_slash_i = np.delete(R, i)
                    dot_prod = np.dot(
