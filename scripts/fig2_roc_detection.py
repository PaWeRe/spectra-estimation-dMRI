...
Narrative (Pillar 1 -- "the collapse"):
  ...
  - The MIRROR ablation -- a 5-feature LR on the inner (intermediate) bins,
    with BOTH the two outer compartments AND the D=20 dump bin REMOVED --
    drops well below (AUC ~0.80-0.82). The inner bins are individually
    informative but redundant once the outer pair is present ("redundancy,
    not uselessness"). The three orange curves are the SAME spectral
    classifier on different bin subsets: solid = all 8, dashed = 2 outer
    (sits on ADC), dotted = 5 inner (sits low).
...
INNER_IDX = [1, 2, 3, 4, 5]  # the five inner (intermediate) bins -> mirror ablation (D=20 excluded)
...
def build_zone(df_zone):
    ...
    X_5 = df_zone[[NUTS_COLS[i] for i in INNER_IDX]].values
    ...
    out = {
        ...
        "lr2": curve_for(X_2, y),
        "lr5": curve_for(X_5, y),   # mirror ablation: 5 inner bins (outer + D=20 removed)
...
def plot_panel(ax, res, title, show_ylabel=True):
    ...
    # subsets: solid = all 8, dashed = 2 outer (sits on ADC), dotted = 5 inner
    # (mirror ablation -- sits well below: redundancy, not uselessness).
    ax.plot(res["lr5"]["fpr"], res["lr5"]["tpr"], color=COLOR_NUTS,
...
def main():
    ...
    # entries are consecutive (8 / 2-outer / 5-inner) so the reader reads them
