...
- Manuscript numbers were already correct; results.tex states the validated decomposition inline. Claim qualified per Ghesu 2021 / Tanno 2021.

**⚠️ ESCALATED 2026-06-06 → OPEN INVESTIGATION (Patrick): the uncertainty biomarker does not hold up.** Rejection-curve + patient-bootstrap (run this session): deferring by probability-CI-width raises accuracy 0.90→0.97 but that ordering ≈ distance-to-boundary (ρ=−0.94), which the MAP point estimate gives for free; the geometry-free **logit-spread rejection curve is flat**, and the misclassified/correct **logit ratio 1.27× has a patient-bootstrap CI [0.94, 1.67] that includes 1** (the earlier Welch p=0.003 was ROI-level and overstated). So in this cohort the propagated interval adds **no statistically reliable error flag beyond the point estimate**. This weakens **Block C (uncertainty biomarker), NOT the spine** (NUTS still justified by identifiability + joint noise inference). **Manuscript edited to the honest interim** (capability-not-biomarker; bootstrap CIs; Welch removed; `\note` on Fig 7/Results). Full plan + alternatives-to-rule-out (calibration, uncertainty-as-features, proper selective prediction, pixelwise low-SNR flagging, confounds, power) in **`notes/uncertainty_investigation_2026-06-06.md`** — Patrick to run a fresh session. **Do NOT submit the uncertainty-as-biomarker claim until this resolves; Fig 7 framing/placement may change.**
