...
### Results — `results.tex` (RESTRUCTURED 2026-06-22 eve → actual cohort results only)
| St | Unit |
|----|------|
| 🔶 | Diffusivity spectra (Fig 2) — spectra-led + key observations; identifiability/CV REMOVED (→ Theory tool, Discussion interpretation). Needs final pass. |
| 🔶 | Tumor detection (Fig 3 ROC) — mirror ablation + redundancy framing; needs final pass |
| 🔶 | ADC ≈ optimal spectral discriminant (Fig 4 correlation) — was "Why ADC works"; MUST drop the sensitivity/mechanism half (Fig 6 → Discussion) |
| ☐ | Spectral shifts by Gleason Grade Group (Fig 5) |
| — | ~~Prediction confidence~~ → MOVED to Discussion (Fig 8 uncertainty) |
| — | ~~Pixel-wise Spectral Decomposition~~ → MOVED to Discussion (Fig 9) |

### Discussion — `discussion.tex` (RESTRUCTURED 2026-06-22 eve — now also home to sensitivity/validation/uncertainty/pixel-wise)
| St | Unit |
|----|------|
