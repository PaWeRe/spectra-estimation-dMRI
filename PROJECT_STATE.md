...
| ✅ | ADC Computation and Classification — MERGED 2026-06-23 (one subsection, one header; ADC-computation ¶ + the already-final Classification ¶¶). Stripped the `fig:sensitivity` forward-ref from the standardization sentence so sensitivity is first-mentioned in Discussion. **Methods figure numbering now MRM-compliant** (body first-mentions run 1–9 in order). |
