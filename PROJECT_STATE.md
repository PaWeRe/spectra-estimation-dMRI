...
**⚠️ FLAG — NOT fixed, needs data verification:** Results "Tissue-Specific Spectra" reports MAP fractions (e.g. PZ-normal $D=3.0$ = **23.5%**) that look like **λ=0.1-era** values; per **F1**, tuned MAP (λ=1e-3) on this cohort gives ≈**0.43** there (NUTS ≈0.48) and Fig 1 shows tuned MAP. The MAP percentages in `results.tex` may be **stale/inconsistent with Fig 1** — verify against `features.csv` and correct before submission. (I cut the most egregious "MAP smears free-water across 1.5–3.0" sentence but left L8's numbers for verification.)

**REMAINING for Sat 2026-06-06:** (1) **verify/fix flagged Results MAP numbers** vs `features.csv`; (2) **final word-count check on Overleaf texcount** + trim last ~200 if needed; (3) **classification waterproofing** (Figs 2/4/9 + Table 1 — Patrick's #2, still open); (4) MRM compliance (data-availability + code SHA already in methods.tex; ORCID, LLM disclosure, keywords); (5) **Sandy** van-Trees validation (TODO markers); (6) **Overleaf sync** — all repo-only.
