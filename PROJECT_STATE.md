...
- **Last update:** 2026-07-26 — **DISCRIMINATIVE-POWER FRAMEWORK REMOVED FROM THE MANUSCRIPT (Patrick's decision, per Stephan's review). This SUPERSEDES the 2026-06-28 "theoretical core" entries below — the framework is NOT in this submission and must NOT be resurrected into it; it is deferred to a follow-up paper.**
  - **Stephan's objection (email):** the formal theory does not belong in this paper (we did not investigate multi-contrast; theory validity uncertain — pathology keys on small features at small volume fraction that we are not sensitive to, and cell density is a secondary marker, so the "biology-limited ceiling" interpretation is not safe). Good for a follow-up / review paper.
  - **Surgical cut (NOT a git reset — a reset would have restored the old over-claim and dropped good work):**
    - **CUT** `theory.tex` §"Discriminative Power of a Diffusion Scan" (Eq `dmax`/`cos`, Mahalanobis/cos²θ machinery).
    - **CUT** `results.tex` §"A discriminative-power ceiling for detection" (in-sample Ledoit–Wolf ceiling + cos²θ≈0.88/0.82; raw draft with open TODOs).
    - **REWROTE** `discussion.tex` §"Grade-dependent spectral shifts" framework-free + conservative: spectrum makes grade biology **visible** (descriptive), **no per-ROI grading advantage over ADC**, **no grading AUC** justified purely by small n (8–9/group). Dropped the `eq:dmax` ref, the unlocked in-sample/LOOCV numbers (0.9→0.8), and the "not reliably recoverable per-ROI" verdict on the D=2.0 bin (Patrick flagged it too harsh → softened to a group-level trend in the less-identified range).
    - **KEPT** the RSIrs/`domingo2025restriction` scope sentence in Limitations (stripped its "the framework above accommodates…" clause).
  - **What SURVIVES and carries the paper's novelty:** the "why ADC works" **explanation** — identifiability (Fisher/`eq:fisher`) + alignment of the ADC sensitivity vector with the LR coefficients (r=−0.79 PZ/−0.88 TZ, `discussion.tex:11`) + the measured ADC≈spectral-discriminant result (ρ≈−0.98, Results). None of this depended on the cut equations.
  - **Left in place (harmless):** `scripts/framework_validation.py` (useful for the follow-up); `references.bib` untouched (domingo2025restriction / he2025improved / panagiotaki2014microstructural still cited elsewhere). Full framework detail preserved in memory `project_discriminative_power_framework` for the follow-up paper.
  - **Pre-existing nit spotted, NOT fixed (Patrick's call):** theory says "we test this alignment directly (Results)" but the vector-Pearson lands in Discussion; behaviorally fulfilled by the ρ≈−0.98 Results finding.
  - **The 06-28 entries below are HISTORICAL — read them as the record of the abandoned framework, not as the current plan.**
- **Superseded 2026-06-28 entry (framework — HISTORICAL):** **RESYNC (this file was 4 commits + one major analysis behind) + the DISCRIMINATIVE-POWER FRAMEWORK is now the validated theoretical core (memory-only, not yet in manuscript or repo).**
  - **(A) Committed since the 06-23 Discussion pass, not previously logged here:**
    1. **97564b0** (06-24) "stephan comments update" — broad polish across abstract / discussion / legends / methods / results / supporting / tables / theory.
    2. **d7b2cae** (06-24) Stephan review round 1 + **inner-fraction ablation BUGFIX (the only computation change): inner set 6→5 bins — dropped the D=20 vascular-pseudodiffusion bin, leaving the 5 intermediate fractions {0.5,0.75,1.0,1.5,2.0}; `recompute.py` + Fig 2 (ROC) regenerated; new NUTS-inner AUC PZ 0.818 / TZ 0.796** (supersedes the 6-bin 0.817/0.775 in the 06-21 entry below). Also: **physiological compartment RENAMING throughout — "outer/inner" DROPPED → restricted (0.25) / intermediate (0.5–2.0) / free water (3.0) / vascular pseudo-diffusion (20); "the restricted and free-water fractions" = the detection-pair handle.** Pixel-wise + directional data flagged as a **separate, earlier cohort** (intro + legends). SI **renumbered by order of mention: atlas=S1, convergence=S2, directions=S3.** Methods: per-GGG counts (1:8, 2:12, 3:5, 4:2, 5:2); σ_R=√10; NUTS runtime ~15–40 s/ROI (~1 h / 149 on M1 Pro); LOOCV fold size clarified.
    3. **3426514** (06-25) Abstract → **250-word MRM cap**, "Theory and Methods" header, equivalence scoped to **ROI-level detection**, localizer numbers woven in (2-frac = 8-frac 0.93/0.94; 5 intermediate alone 0.82/0.80).
    4. **9fb4608** (06-27) Abstract broadened per Patrick/Stephan (DWI defined; gap reframed to "which compartments the signal can resolve + why one scalar works"; tentative grade-shift clause; posterior-null) + **Wells affiliation 3 (CSAIL, MIT) added to `main.tex`**; stored word count 247→250. ⇒ the **Abstract was materially rewritten twice → §0 tracker Abstract rows updated to 🔶 (need a final pass once the framework lands).**
  - **(B) Memory-only, NOT committed, NO script saved (reproducibility gap):** the **discriminative-power framework** — developed 06-27, empirically validated 06-28 — is the proposed **THEORETICAL CORE** (Patrick's call). Master eq **d²_max = Δᵀ(Σ_bio+Σ_est)⁻¹Δ**; readout d²(w) = d²_max·cos²θ; four quantities (contrast Δ = biology · identifiability F = UᵀU/σ² = acquisition · sensitivity w = algorithm · **biological heterogeneity Σ_bio = the term the manuscript omits**). **Crux: the binding limit is BIOLOGICAL (Σ_bio), not instrumental** — biological var ≈ 13× estimation var in the discriminative direction; measurement-noise-only counterfactual AUC = **1.000**, so the entire 0.95→1.0 gap is irreducible biological overlap; ADC is blind only where the data is blind (its sensitivity null at D≈1.5 = the acquisition's identifiability null) ⇒ ADC≈spectrum, richer inversion can't help, only **NEW CONTRAST (diffusion time / T2)** can. Full detail + validation numbers + caveats in memory `project_discriminative_power_framework`.
    - **Detection-vs-grade geometry validated** (`project_detection_vs_grade_geometry`): both axes restricted-dominated, ~55° apart, both ADC-aligned; **the bolder "grading is acquisition-limited" thesis is FALSIFIED — do NOT resurrect**; honest = ADC near-optimal for BOTH detection and grading.
    - **RSIrs 2025 scope boundary** (`project_rsirs_2025_scope_boundary`): Domingo/Seibert *J Urol* n=1892 shows ADC≈chance for *patient-level automated* detection → **scope our claim to ROI-level / localized**; not a contradiction (different task + min-ADC comparator), confirms the restricted-compartment story.
    - **Langkilde novelty** (`project_langkilde_equivalence_novelty`): our cohort's 2018 source paper already published the equivalence → our novelty is the **EXPLANATION** (identifiability + alignment); never headline the equivalence as new.
  - **(C) Separate in-flight stream (untracked, none committed):** Zenodo data deposit — `zenodo_data_release/`, `paper/drafting/zenodo_data_deposit_draft.md`, `_email_draft_2026-06-19_zenodo.md` (~06-19); + a batch of literature PDFs added to `assets/`; + `scripts/fig3_battery_prototype.py`; + `paper/figures/figS_recovery.{pdf,png}`.
  - **THIS SUPERSEDES "§D = next step is the Discussion pass":** the framework reframes Theory (centerpiece), Discussion lines 16/19, Fig 7 caption, Fig 1(d) SVD panel, abstract, intro, + CV hygiene (demote CV → descriptive; lead with CRLB/posterior-std + SVD rank).
  - **REPRODUCIBILITY GAP CLOSED (2026-06-28):** the framework analysis is now persisted as **`scripts/framework_validation.py`** (reads the gold `results/biomarkers/features.csv`; writes `framework_validation.csv` per-bin + `framework_validation_summary.csv`; run `uv run python scripts/framework_validation.py`). **All 12 headline comparisons reproduce (OK):** detection ceilings PZ 0.961 / TZ 0.988 (within 0.01 of ADC 0.951/0.979); counterfactual AUC = 1.0000 both zones; grade in-sample ceiling 0.918 → honest LOOCV 0.80 ≈ ADC 0.811; effective d'² 5.45/1.56, ratio 3.50 = contrast² 1.86 × heterogeneity 1.89. **Two numbers are illustrative not exact (honest, NOT tuned):** the per-bin Σ_bio/Σ_est *magnitudes* (regime split — 3 biology-limited / 5 measurement-limited — reproduces exactly; ratios run a little below the in-session 19.7/2.4/4.7 because Σ_est = mean posterior variance here) and the counterfactual *d'² magnitude* (≈57/110 vs memory ~71/132; the AUC→1.0 conclusion is robust). NOT yet committed.
  - **THEORY FRAMEWORK SUBSECTION DRAFTED (2026-06-28, cont.):** added `\subsection{Discriminative Power of a Diffusion Scan}` to `theory.tex` (placed after "ADC as a Spectral Functional", which it generalizes). Introduces the master eq **d²_max = Δᵀ(Σ_bio+Σ_est)⁻¹Δ, AUC_max=Φ(d_max/√2)** (`eq:dmax`) + the readout bound **d²(a)=d²_max·cos²θ** (`eq:cos`); ties **Σ_est≈F⁻¹** back to the existing Fisher Eq (`eq:fisher`); names the four factors (biology Δ / acquisition F / readout cosθ / heterogeneity Σ_bio); states the **three testable predictions** (scalar near-optimal when contrast well-identified+aligned, and ADC's insensitive bins = the unresolvable bins → forfeits nothing; binding limit = whichever of Σ_bio/Σ_est dominates along Δ → biology-limited here; richer inversion only shrinks Σ_est so can't raise a biology-limited ceiling → need NEW CONTRAST). **Citations wired (Patrick's ask):** added **`domingo2025restriction`** (RSIrs, *J Urol* 2025;214(3):259–271, Rojo Domingo…Dale/Seibert — was MISSING from `references.bib`); cited it + `panagiotaki2014microstructural` (VERDICT) as the new-contrast levers, and `he2025improved` (already in bib) as the richer-fit-of-fixed-acquisition foil. **WORD COUNT:** adds ~310 words + 2 display eqs → body likely now **>5000** (was ≈4907); Patrick to verify texcount on Overleaf and decide trims (the existing identifiability/CV prose is the de-dup candidate). **Deliberately NOT done (post-supervisor):** de-dup the framework vs the existing identifiability/CV story (demote CV→descriptive; lead CRLB/posterior-std + SVD rank); Fig 1(d) SVD panel; Results/Discussion rework to TEST the 3 predictions (fed by `framework_validation.py`); abstract/intro reframe. **ABSTRACT: per Patrick the last two abstract commits (3426514/9fb4608) are NOT good — he will revert/rework them himself; do NOT touch.**
  - **RAW FIRST-DRAFT WIRING ADDED (2026-06-28, so the framework isn't dangling on a supervisor read):** new Results subsection `A discriminative-power ceiling for detection` empirically tests Eq~10–11 (in-sample Ledoit–Wolf Mahalanobis ceiling PZ 0.96 / TZ 0.99; ADC within 0.01 → cos²θ≈0.88/0.82; counterfactual Σ_est-only → AUC≈1.0 ⇒ **biology-limited**) — flagged raw with an inline `% NEW` + `(@Stephan @Sandy)` note. Discussion Limitations gains ONE RSIrs scope sentence (`domingo2025restriction`: equivalence scoped to ROI-level/localized; patient-level delineation-free = a different task). **Discussion §Grade-dependent spectral shifts REFRAMED (correctness-critical — grading haziness was the trigger for reopening):** dropped the now-FALSIFIED "any signal beyond ADC lives in the intermediate bins → decomposition adds value there" claim; new position = the spectrum **EXPLAINS** grade biology but gives **NO per-ROI grading advantage over ADC** (grade axis dominated by the well-identified restricted D=0.25 bin ADC already reads; the D=2.0 epithelial shift is a real GROUP trend but poorly identified per-ROI; in-sample grade ceiling ≈0.9 collapses to ADC ≈0.8 under LOOCV = small-sample overfitting; field grading gains come from NEW acquisition contrast — diffusion time / relaxation — not richer inversion). **OPEN reconciliation:** Discussion intro line 4 ("advanced models … improving on it only for grade stratification" `he2025improved`) mildly contradicts the new grading position — reconcile in the rework. **CV left as-is** (Patrick flags it mean-dependent/suboptimal in the email; principled replacement = Σ_est≈F⁻¹; full demote deferred to next week). The inline `@Patrick`/`@Stephan` Discussion questions (1-D-collapse-vs-ill-conditioning; sensitivity×contrast elaboration; PC1) are ANSWERED by the framework but left untouched (supervisor-contingent rework).
  - **NEXT: Patrick reviews diffs on GitHub + compiles `main` in Overleaf, then emails supervisors to run the framework by them.** **OPEN:** word-count trim decision; the post-supervisor rework list above; Zenodo thread sequencing.
- **2026-06-23 (cont. 4) — DISCUSSION PASS EXECUTED + Fig 6–9 LEGENDS TRIMMED + INTRO THINNED + BIB CLEANED + LIT MAP BUILT. Committed + pushed; Overleaf recompile + Patrick review pending.** (1) **`discussion.tex` rewritten** figure-anchored (Patrick's order): lead-in + **§Why ADC works** (Fig 6 — rock-solid mechanism: ADC and the spectral discriminant are both ~linear functionals of the same spectrum; cohort detection variation is ~1-D on the two outer bins; two readouts onto one axis ⇒ near-identical ROI ranking; intermediate bins *redundant not useless* = sensitivity×contrast) + **§Grade-dependent shifts** (Fig 7) + **§What the Bayesian posterior contributes** (MAP≈NUTS + 3-tier identifiability CV + Fisher two-gain + uncertainty NULL + confidence-for-free, Fig 8) + **§Pixel-wise feasibility** (Fig 9 + re-referenced SI `fig:directions`, fixing the orphan) + **§Limitations and future directions** (incl. **F12** radiological-target positives 11/40; T2/LWI future). **`\iffalse` STAGING BLOCK DELETED.** (2) **Fig 6–9 legends trimmed** to "what you see" (interpretation/numbers → body). (3) **Intro thinned**: dropped PI-RADS v2 (`weinreb2016`; kept v2.1 `turkbey2019`), deduped IVIM cite, **wired in `wells2022estimation`** (the direct predecessor). (4) **`references.bib` cleaned**: deleted `delong1988`/`rozenberg2016`/`aksitciris2019`/`betancourt2017`/`hanley1982`/`weinreb2016`; **commented out `maier2004`** (wrong brain-tumor paper + `\todo` — **Stephan to supply the JMRI 4-compartment prostate paper**); fixed `neal2011`→`@incollection`; **added** `he2025improved` (ally: detection≈ADC, grading better), `johnston2019verdict`, `chatterjee2018hybrid`, `chatterjee2015changes`, `sabouri2017luminal`. **Verified 31 cited = 31 active, zero missing/uncited.** (5) **Literature map** at `notes/discussion_lit_map_2026-06-23.md` (figure-anchored point-map + weight-ranked lit w/ verified OpenAlex counts + fetch list + bib/legend actions). (6) **Body ≈4907/5000 words** (tight; Discussion ≈1413 — verify texcount on Overleaf, trim if over). **Per Patrick's steer NOT engaged head-on:** Singh/INNOVATE uncited (need PDF; ~36 cites), cautious on beats-ADC group. **NEXT:** Patrick reviews narrative+citations in Overleaf → submission roadmap (Conclusion → Abstract → SI legends → DAS/GitHub/Zenodo/SHA-1 → cover letter → co-authors). **OPEN:** Singh INNOVATE PDF? `maier2004` replacement (Stephan); optional PC1/variance-explained analysis to make the 1-D detection axis a *measured* fact (Patrick's call).
- **2026-06-23 (cont. 3) — RESULTS REFINED + ALL GATED COMMENTS REMOVED + DISCUSSION-PASS PLAN WRITTEN (deferred to fresh session, Patrick's call).** Results Fig 3/Fig 4 descriptions refined per Patrick (zonal+tissue nuance; weak-bin nuance) — see §0 Results rows. Removed all 4 `\stephan{}` review comments (figure_legends, discussion ×2, introduction) — co-authors review the finished draft instead. **The Discussion pass + the rest of the submission roadmap are captured in the new section "## D. Discussion pass + submission roadmap" below; do that as the next (fresh) session.** Committed + pushed.
- **2026-06-23 (cont. 2) — RESULTS PASS DONE → 3 lean subsections.** `results.tex` now = (1) Diffusivity spectra (Fig 3), (2) Tumor detection (Fig 4 ROC), (3) ADC equals the spectral discriminant (Fig 5, ranking correlation only). CUT from Results and **parked verbatim** in an `\iffalse...\fi` STAGING AREA at the bottom of `discussion.tex`: the sensitivity-mechanism half, the GGG subsection, the prediction-confidence subsection, and the pixel-wise subsection — **plus** the two now-outdated Methods subsections (ADC Sensitivity Analysis; Pixel-wise/Direction-wise), which are now DELETED from `methods.tex` (Methods = the target **3 subsections**). Spectra/ROC/ADC-discriminant **legends trimmed** to "how it's visualized." Figure first-mentions verified to run **1–9 in numeric order** in the rendered body (sensitivity/GGG/uncertainty/pixel-wise are first-mentioned by the existing Discussion paragraphs). **Transitional TODO:** SI `fig:directions` now unreferenced in the rendered body (refs parked) → re-reference in the Discussion pass. The staging prose is RAW MATERIAL to merge into the existing Discussion paragraphs (which already cover GGG-histology/uncertainty/pixel-wise) — then delete the staging block. Committed + pushed. Overleaf recompile pending. **NEXT: Discussion pass.**
- **2026-06-23 (cont.) — STRUCTURE LOCKED + FLOAT REORDER DONE + METHODS §Patient-Cohort FINALIZED + DATA-PIPELINE VERIFIED.** New manuscript skeleton agreed (Patrick): **Methods → 3 subsections** (Patient Cohort & MRI Acquisition · Spectral Estimation [+ validation/simulation figure for λ-tuning justification] · ADC Computation **merged with** Classification); **drop** standalone ADC-Sensitivity + Pixel-wise/Direction-wise from Methods (sensitivity method already in Theory; pixelwise/direction method sentences move with their Discussion paragraphs). **Results → 3 lean subsections** (spectra · detection/ROC · ADC≈discriminant). **Discussion** carries sensitivity *mechanism* + GGG/grading + uncertainty + pixel-wise. **Figure order RESTRUCTURED + executed in figures.tex:** 1 Fisher (Theory) · **2 validation (Methods, moved up)** · 3 spectra · 4 ROC · 5 ADC-discriminant (Results) · 6 sensitivity · **7 GGG (moved to Discussion)** · 8 uncertainty · 9 pixel-wise (Discussion). Only floats 2↔3 physically swapped; rest keep slots; `\ref` auto-syncs. **DATA-PIPELINE VERIFICATION (Patrick asked to double-check):** extraction = arithmetic voxel-mean per b-value (`utils/process_img_data.py`), trace = geometric mean of 3 directions upstream; ALL counts reconcile (149 ROIs, 109 normal/40 target, PZ 81/TZ 68, grading n=29 GGG1=8/2=12/≥3=9, PZ21/TZ8). **KEY FINDING (F12 below):** the 40 "tumor" positives are mpMRI *radiological targets*, only **29/40 biopsy-confirmed cancer** (7 benign GGG0, 1 HGPIN, 3 no Gleason). **DECISION (Patrick): keep all 40, reframe honestly** — Patient-Cohort text rewritten accordingly; **Discussion Limitations must note radiological-target positives** (ties to circularity). **F13:** two `4+3+5` cases (new03 PZ, new45 TZ) coded GGG 5 vs ISUP-standard GGG 3 — both in GGG≥3 bucket so Fig 5/binary unaffected; only continuous-GGG ρ shifts marginally → **Stephan to confirm** tertiary-pattern coding. Committed + pushed. Overleaf recompile pending (Patrick). **NEXT: Methods §Spectral Estimation** (light edit + wire/relegend validation as Fig 2).
- **2026-06-23 — THEORY + FIG 1 CLOSED.** (1) Stephan's Fig 1 fixes applied to `scripts/generate_fisher_figure.py` (in-cell numbers 11→14; titles 16→19, padded, de-bolded; panel (b) title two lines) — regenerated `fig_fisher_v2.{pdf,png}`, all values byte-identical, committed+pushed (38acaa7). (2) Fig 1 legend trimmed to "how it's visualized" — observations (>0.99 correlation; D=0.25/≥3.0 noise-floor; prior+constraint gain magnitudes) now live in Theory only; terminology aligned (van-Trees Gaussian-prior framing, Eq.~\ref{eq:fisher}, σ_R=3.16). (3) All three Theory subsections marked ✅ (incl. ADC-as-functional, reviewed unchanged). **NEXT (per Patrick): Methods finalization + the figure float-move/renumber pass** (Theory=Fig 1; Results=2–5; Discussion=6–9 in figures.tex/figure_legends.tex — numbers auto-sync via `\ref`, only float ORDER must move). Overleaf recompile pending (Patrick).
- **2026-06-22 (eve)** — **MANUSCRIPT-WIDE RESTRUCTURE decided + Theory reworked (Patrick steer after Overleaf recompile).** Principle: each of Theory/Methods/Results/Discussion has ONE purpose, non-repetitive; figures attributed by what they are *for*; figure legends = "how it's visualized" only, observations live in the text. **Theory → 3 subsections (from 5):** (1) *Signal Model and Discretization* (merges Signal Model + Identifiability/Discretization; softer phrasings — "can be modeled", "conditioning can guide the grid", heuristic = "loose design guide combined with physiological knowledge"; now **references Fig 1a + 1c** as stage-setting); (2) *Estimation* (merges MAP + Bayesian; framed as **two estimators from one non-negative model** — MAP mode + full posterior; ends with **Fig 1b** as the tractability tool: regularization gain + non-negativity gain); (3) *ADC as a Spectral Functional* (unchanged). **Fig 1 (Fisher) now LIVES IN THEORY** — it is stage-setting/pre-data, not a result (this *reverses* the earlier "no figures in Theory" edit, per Patrick's call). **Results §1 → "Diffusivity spectra"** (spectra-led, Fig 2): SNR + cohort spectra + key observations (outer bins dominate + clearest tumor–normal difference; intermediate bins near zero / hard to estimate); **identifiability/CV REMOVED from Results** (→ Theory for the tool, Discussion for interpretation). **Figure RELOCATIONS decided, renumber NOT yet executed in figures.tex/figure_legends.tex:** Fisher→Theory; spectra/ROC/ADC-discriminant/GGG→Results; **sensitivity (old 6) + validation (old 3) + uncertainty (old 8) + pixel-wise (old 9) → Discussion.** **Lit check done** (`notes/lit_check_uncertainty_propagation_2026-06-22.md`): Q1 posterior-through-classifier = partly novel → lead as a **NULL result**; Q2 classifier "confidence for free" = **STANDARD**, don't overclaim (calibration/DCA/selective-prediction/conformal). **DONE this turn:** Theory restructure + Results-spectra rewrite (committed). **NOT yet done:** figure float moves + renumber; Results detection/ADC-equivalence/GGG rework (drop sensitivity from "Why ADC works"); Discussion rework (sensitivity mechanism + validation + uncertainty + pixel-wise; resurface the 3-tier per-bin CV that left Results — well 0.25/3.0/20 = 0.20/0.32/0.36; intermediate 1.5/2.0 = 0.74/0.52; poor 0.5/0.75/1.0 = 0.81–0.83). Overleaf sync pending (Patrick).
- **2026-06-22 (eve, cont.) — Theory phrasing FINALIZED + next-session TODOs.** Applied Patrick's Overleaf feedback to `theory.tex`: (1) "choice of grid" → "choice of the diffusivity grid"; (3) heuristic ending → "treat this analytic rule as a **heuristic** and combine it with **biological** knowledge of the relevant water compartments"; (4+5) subsection retitled **"Bayesian Spectrum Estimation"** with an explicitly-probabilistic opener (observation model + non-negativity + prior → posterior; two estimators = MAP mode + full posterior); (7) HMC now explained properly (auxiliary momenta + gradient trajectories) + Gibbs reframed as "implemented but mixed too slowly"; (8) CV statement softened to a continuous interpretation (NOT a "CV ≪ 1" rule; the 3-tier categorization stays OUT of Theory → Results/Discussion). **Theory section considered DONE pending Patrick re-read.** **NEW open TODOs for the fresh session:**
  - **(a) [point 2] Sampler oscillation between neighboring intermediate bins.** The sampler swaps weight between adjacent intermediate bins (anti-correlated; visible in **S2** posterior diagnostics) — a direct manifestation of the Fisher off-diagonal collinearity ("trade off against one another almost freely"). Mention in Discussion (and/or briefly in Bayesian Spectrum Estimation / the S2 caption). Not addressed in Theory yet.
  - **(b) [point 5] Make the posterior explicit?** Decide whether to write the joint posterior explicitly (Gaussian observation model × half-normal prior on $\Rvec$ × half-Cauchy on $\sigma$) and align the formulation with **Sandy's ISMRM-2022** abstract (`assets/ISMRM-2022-abstract.pdf`). Currently left implicit (priors + likelihood are written as separate equations but the posterior is not stated as one line).
  - **(c) [point 6 — answered in chat, no manuscript change needed] Augmented-system / NNLS / active-set.** Implementation = `scipy.optimize.nnls` on the stacked system $[\Umat;\sqrt\lambda I]\Rvec=[\svec/S_0;0]$ (Lawson–Hanson active-set); see `biomarkers/recompute.py:80` and `models/prob_model.py:113`. Consider a one-line Methods gloss only if a reviewer would want the detail.
  - **Then the big next pass (unchanged):** figure float moves + renumber (Theory=Fig 1; Results=Fig 2–5; Discussion=Fig 6–9) in `figures.tex`/`figure_legends.tex`; Results detection + ADC-equivalence + GGG rework (split "Why ADC works" — keep the correlation result in Results, move the sensitivity *mechanism* Fig 6 to Discussion; drop the Prediction-confidence + Pixel-wise subsections); Discussion rework (sensitivity mechanism + sampler validation + uncertainty NULL-result framing + pixel-wise; resurface the 3-tier per-bin CV); legends trimmed to "how it's visualized" with observations moved to text.
- **2026-06-22 (pm)** — **Identifiability story de-duplicated across Theory/Results/Discussion/legend; re-gridding robustness test added; σ_R/λ disambiguation; CRLB cite resolved.** Division of labor (per MRM guidelines `paper/drafting/mrm_author_guidelines.md` + Stephan's fewer-subheadings advice): **Theory** = tool only (ill-posed inverse → Fisher matrix → CRLB def → ΔD∝D^{3/2} design principle; the CRLB-overestimate/decomposition paragraph REMOVED); **Results** = Fig 1 findings + the bin-rule adherence test + Fig 1b magnitudes + the new robustness check; **Discussion** = interpretation only (the "data-limit-not-artifact" conclusion moved here; the design-principle re-derivation dropped); **legend** = descriptive only. **Re-gridding experiment** (`scripts/regrid_robustness.py`, tuned-MAP on 3 grids, read-only, nothing committed from it): detection AUC + ADC↔discriminant ρ essentially unchanged on the rule-prescribed and finer-low-D grids, tumor-normal contrast stays at the two outer bins → **main claim ROBUST to discretization**, confirms theory.tex's "regardless of bin placement" empirically. Detection only (n=81/68); says nothing about grading (n=29). **λ-naming:** the half-normal prior is now parameterized by its scale σ_R=3.16 (precision 1/σ_R²=0.1); λ is reserved for the MAP ridge weight (10⁻³). They are separate hyperparameters (theory:88 "linking" sentence fixed; Methods states both). **CRLB cite:** dropped the vague "pessimism" phrasing; ill-conditioning now anchored on **Jalnefjord 2019 MRM** (`jalnefjord2019optimization`, added to `references.bib`). Bin values/biology cite via Langkilde 2018 + Maier 2022 + Panagiotaki 2014 (VERDICT) + Conlin 2021 (RSI). **He 2025** (`assets/s00261-024-04684-z.pdf`) = the 4-bin / odd-D grid Stephan rejected. Committed + pushed. Overleaf sync pending (Patrick). **NOTE:** parallel session may still be iterating PROJECT_STATE / subparagraphs — reconcile if its edits land.
- **2026-06-22 (am)** — **Fig 1(b) van-Trees CRLB framing DECIDED; identifiability text reworked; Sandy gate removed.** Decision (Patrick): bar 2 is the van-Trees bound under a *Gaussian approximation of the half-normal prior* (precision λ=0.1) — renamed from the inaccurate "Bayesian CRLB **with the half-normal prior**" so that NUTS (bar 3, true half-normal + non-negativity) sitting *below* it reads as the **constraint gain**, not a CRLB violation. The prior/constraint split is now stated as **λ-dependent and non-orthogonal** (both contributions real, same direction; decomposition kept). Manuscript states this as our position; **Sandy reviews the final draft rather than gating it** (derivation write-up: `notes/CRLB_NOTE_FOR_SANDY_v2_2026-06-21.md` — do NOT rephrase per Patrick). Edited: `figure_legends.tex` (Fig 1 caption + dropped `\sandy`), `theory.tex` (Identifiability ¶ tightened + renamed), `discussion.tex` (¶5 renamed + λ-note, dropped `\sandy` + TODO), `scripts/generate_fisher_figure.py` (bar label → "Bayesian CRLB (Gaussian prior)", docstring/print updated), regenerated `fig_fisher_v2.{pdf,png}` (numbers unchanged). Grounded in **F8** (NUTS posterior robust to λ over a 30× range → λ=0.1 defensible, no recompute needed) + **F10**. Committed + pushed. Overleaf sync pending (Patrick). **NOTE:** a parallel session was iterating the §0 tracker / subparagraphs concurrently — reconcile if its PROJECT_STATE edits land.
- **2026-06-21** — **Reviewer-feedback round on the sensitivity figure (Fig 6) + new §0 manuscript-finalization tracker.** A non-domain ML reviewer read Fig 6 as ADC and the spectral classifier being two *different* estimators (ADC sensitive to the middle bins; classifier ignores them). Resolved as a communication issue, not a results issue: (1) added the **mirror ablation** — inner-6 LR with the two outer bins removed drops to NUTS PZ 0.817 / TZ 0.775, vs outer-2 ≈ full ≈ ADC — to Table 1, the ROC (Fig 4, `fig2_v3`, new orange dotted curve), and `recompute.py`; also a **4-poorly-id (CV>0.7) probe** → AUC 0.82/0.81 cited in Results as the "redundancy not uselessness" punchline. (2) Reframed Results "Why ADC works" around **sensitivity × contrast** (ADC *is* sensitive to the mid bins, but that sensitivity is diagnostically inert there because the mid bins carry little tumor–normal contrast and are poorly identified) — replaces the inaccurate "carry little weight in either method" line; ROC + Fig 6 captions updated. (3) Expanded **Methods → Classification** standardization rationale (z-scoring how + why; Stephan had been confused) — **FINALIZED** this pass. Commits 66ca436 + 002f36b, pushed; Overleaf sync pending (Patrick). **New §0 below** tracks per-subsection finalization going forward.
...
**Process.** Walk every prose unit below in document order; polish one, then mark it. Statuses: ☐ pending · 🔶 revised this session, needs a final pass · ✅ reviewed + finalized (the bar: like Methods→Classification, 2026-06-21). Captions in `figure_legends.tex` / `table_legends.tex` get polished alongside their parent float, not separately. Figure→section attribution (**NEW 2026-06-22 eve** — renumber NOT yet executed in figures.tex/figure_legends.tex): **Theory** = Fig 1 Fisher (stage-setting). **Results** = Fig 2 spectra · Fig 3 ROC · Fig 4 ADC-discriminant · Fig 5 GGG. **Discussion** = Fig 6 sensitivity · Fig 7 validation · Fig 8 uncertainty · Fig 9 pixel-wise. SI = atlas, directions. (Old order was: 1 Fisher · 2 spectra · 3 validation · 4 ROC · 5 ADC-discriminant · 6 sensitivity · 7 GGG · 8 uncertainty · 9 pixel-wise.)

**Progress: 4 ✅ / 33 body units** (Theory closed 2026-06-23; +3 🔶 in Results awaiting their pass). Fig 1 (Fisher) figure + legend CLOSED 2026-06-23.
...
*🔶 materially rewritten twice (06-25 → 250-word MRM cap, scope→ROI-level detection; 06-27 → broadened "why ADC works" + grade-shift clause + posterior-null). Needs a final pass once the discriminative-power framework lands — see the 2026-06-28 entry.*
...
### Theory — `theory.tex` (was 3 subsections; **🔶 REOPENED 2026-06-28** — 4th subsection added for the discriminative-power framework)
| St | Unit |
|----|------|
| ✅ | Signal Model and Discretization — merged (Signal Model + Identifiability/Discretization); "can be modeled"; "conditioning can guide the grid"; heuristic = loose guide + physiology; refs Fig 1a/1c. Finalized 2026-06-23. |
| ✅ | Bayesian Spectrum Estimation — merged (MAP + Bayesian); two estimators from one non-negative model; ends with Fig 1b tractability (regularization + non-negativity gains). Finalized 2026-06-23. |
| ✅ | ADC as a Spectral Functional — finalized 2026-06-23 (content unchanged, reviewed). |
| 🔶 | **Discriminative Power of a Diffusion Scan — NEW 2026-06-28.** Master eq (`eq:dmax`) + readout bound (`eq:cos`); four factors; three testable predictions; Σ_est≈F⁻¹ ties to `eq:fisher`; cites VERDICT/RSIrs (new contrast) vs He (richer fit). Needs: Patrick + supervisor review; de-dup vs the existing identifiability/CV prose (demote CV→descriptive); word-count trim; Results/Discussion wiring of the 3 predictions. |

### Methods — `methods.tex` (RESTRUCTURED 2026-06-23 → 3 subsections)
| St | Unit |
|----|------|
| ✅ | Patient Cohort and MRI Acquisition — finalized 2026-06-23, conciseness pass (Patrick): data-pipeline provenance (radiologist mpMRI seg → ROI voxel-mean → matched Gleason); "tumor" terminology (dropped "mpMRI target" jargon) keeping the honest 29/40-confirmed breakdown (F12); **legacy SNR formula `150√(N/16)` CUT entirely** — it gave median 465, whereas the reported **303 (IQR 176–478) is the NUTS-inferred per-ROI SNR (1/σ̂)**, now stated in Spectral Estimation. Formula is a Gibbs-era artifact, unused by the sampler. |
...
| ✅ | ADC Computation and Classification — MERGED 2026-06-23 (one subsection). Stripped the `fig:sensitivity` forward-ref → sensitivity first-mentioned in Discussion; body first-mentions run 1–9 in order. Conciseness pass: ADC = **log-linear** lstsq (verified `adc_baseline.py`: `np.polyfit` deg-1 on the 5 b≤1000 values 0/250/500/750/1000), single PI-RADS cite (v2.1 `turkbey2019prostate`; dropped v2 `weinreb2016pirads`), wording confirms monoexponential + b_max 1000 = what PI-RADS v2.1 specifies. |
...
### Discussion — `discussion.tex` (RESTRUCTURED 2026-06-22 eve — now also home to sensitivity/uncertainty/pixel-wise)
**Discussion pass = merge the `\iffalse` STAGING AREA (bottom of discussion.tex, parked 2026-06-23) into the existing paragraphs below, then DELETE the staging block.** Existing rendered ¶¶ already cover GGG-histology (¶ "[Stephan-gated]"), uncertainty-null (¶ after MAP/NUTS), Fisher (¶5 ✅), pixel-wise (in Limitations area). The staging holds the richer Results prose (sensitivity-mechanism numbers, GGG bin trajectories, prediction-confidence detail, pixel-wise uncertainty map) + the parked Methods method-sentences. Also re-reference SI `fig:directions` here.
| St | Unit |
|----|------|
| ☐ | ¶1 Central finding: ADC ≈ optimal spectral discriminant |
| ☐ | ¶2 ADC explained not obsoleted; added-value areas |
| ☐ | ¶ Why-ADC-works MECHANISM (Fig 6 sensitivity) — MOVED from Results; relate LR weights ↔ ADC sensitivity vector |
...
**Discussion-content guidance (Patrick, 2026-06-22 eve) — for the uncertainty + pixel-wise paragraphs:**
- *Uncertainty (Fig 8):* explain the null nuance in detail — the posterior adds little partly because outer-bin uncertainty is low anyway; D=3.0 CV *is* high yet still does not propagate to a useful classification signal (we accept this, don't fully know why). Separately *do* highlight that the logistic classifier gives a **prediction confidence for free** — a per-ROI cancer likelihood / distance-to-boundary, not just yes/no — framed as a usable property, NOT a Bayesian-posterior justification and NOT a methodological novelty (lit check: standard; see `notes/lit_check_uncertainty_propagation_2026-06-22.md`).
- *Pixel-wise (Fig 9):* showcase feasibility + appearance — low-D (D=0.25) fraction map ≈ ADC, and D=3.0 map is its negative; include the uncertainty map. Phrase carefully: exploratory; the uncertainty does not propagate to detection (the way we do it); accept for now but show we tried properly; the uncertainty remains interesting for which spectral buckets are reliable.
...
## D. Discussion pass + submission roadmap (written 2026-06-23 — ⚠️ SUPERSEDED as the immediate next step by the 2026-06-28 framework entry; the execution steps + submission roadmap below still apply, but Theory-around-the-master-equation comes first and reshapes the Discussion/abstract/intro framing)

**Context:** Theory ✅, Fig 1 ✅, Methods ✅ (3 subsections), Results ✅ (3 subsections), figure floats reordered (1–9), all gated `\stephan{}` comments removed. The Discussion is the **last big main-body rework**; deferred to a fresh session (Patrick's call) because it needs clean context for a careful literature check + precise high-risk citations. The raw material to merge sits in the `\iffalse...\fi` **STAGING AREA at the bottom of `discussion.tex`**.

**MRM rules that bind this pass** (`paper/drafting/mrm_author_guidelines.md`): body ≤ **5000 words incl. appendices, EXCLUDING captions/abstract/title/tables**; **subheadings allowed** (Stephan: use few); **Conclusions may merge into "Discussion and Conclusions"** (separate short conclusion optional); **avoid bulk citations in the Introduction**; no fixed citation limit but excess hurts.

### D1. Discussion rework — execution steps
1. **Merge the STAGING block into the Discussion, then DELETE the staging block.** Existing rendered ¶¶ already cover histology/GGG, the uncertainty null, Fisher (¶5 ✅), and pixel-wise; the staging holds the richer numbers/sentences to fold in.
2. **Subsections — use sparingly** (MRM allows; Stephan wants few). Proposed: a **"Why ADC works"** subsection that goes Fig 5 (the ranking correlation = *what* happens) → Fig 6 sensitivity (*why*). Possibly one more; otherwise paragraphs. Decide during the pass; drop subsections if they feel forced.
3. **Fig 6 is hard — explain it carefully for the reader, but concise.** Walk it in order: (a) what the ADC **sensitivity vector** $\partial$ADC$/\partial R_j$ is and how the panel visualizes it (sign-flipped, plotted against the LR coefficient vector so aligned bars = matching direction); (b) the **LR weights** (only $D=0.25$ & $3.0$ have bootstrap CIs excluding zero); (c) the **coefficient of variation** overlay (identifiability); (d) a brief **PZ vs TZ** difference (e.g., $D=1.0$ a bit higher in one zone — minor). Core message: discriminative power = sensitivity × tumor–normal contrast → spectrum collapses to the two co-moving outer bins → one scalar (ADC) suffices; not an impoverished signal.
4. **Resurface the 3-tier per-bin CV** (left Results): well-identified $D=0.25/3.0/20 = 0.20/0.32/0.36$; intermediate $1.5/2.0 = 0.74/0.52$; poor $0.5/0.75/1.0 = 0.81$–$0.83$.
5. **Uncertainty (Fig 8) = exploratory side-tool + a NULL RESULT.** State plainly: propagating the NUTS posterior through the classifier gave **no** detection improvement (the negative result must be in). Separately: the logistic classifier gives **prediction confidence for free** (distance-to-boundary) — a usable property, NOT a Bayesian-posterior justification, NOT a methodological novelty (lit check: standard — `notes/lit_check_uncertainty_propagation_2026-06-22.md`). Nuance: posterior adds little partly because outer-bin CV is already low; $D=3.0$ CV is high yet still doesn't propagate to signal (accept, don't fully explain).
6. **Pixel-wise (Fig 9) = exploratory feasibility side-tool.** Low-$D$ ($0.25$) map ≈ ADC; $D=3.0$ map ≈ its negative; include the per-voxel uncertainty map (8-bin > 2-bin uncertainty). Phrase as exploratory; uncertainty doesn't propagate to detection (accept for now); the per-voxel SNR caveat (skews toward tumor) stays.
7. **Directional comparison = ONE sentence** sanity check + ref the SI figure (`fig:directions`) showing the subset of patients. Do NOT over-emphasize. **This also fixes the current transitional orphan** (`fig:directions` is unreferenced in the rendered body right now).
8. **Conclusion** (`conclusion.tex`): decide keep-short vs merge into "Discussion and Conclusions" (MRM allows merge). Adapt to the reworked Discussion; avoid excessive claims.
9. **Finalize the Discussion-figure legends** (sensitivity Fig 6, GGG Fig 7, uncertainty Fig 8, pixel-wise Fig 9) to "how it's visualized" as each parent ¶ is finalized.

### D2. Literature check — Discussion **and** Introduction together (thorough)
- **Introduction:** remove bulk citations (MRM); make it consistent with the reworked Discussion. (Reduce stacked cites, e.g., line 3 two PI-RADS → one, as done in Methods.)
- **High-risk framing (be precise):** cite **influential works claiming to improve on / beat ADC** that our *detection* equivalence contradicts — this is the high-risk, high-interest claim. State carefully that we are in tension with those (for detection).
- **Detection-vs-grading group:** cite works on ADC's grading limitations; position our grading result as **"signs of life"/promise** (tentative, $n=29$) while **detection is well-described by ADC without being an impoverished signal**.
- Use existing pointers: memory `reference_lit_pointers_2026-05-22` (Wang Y 2024, Sjölund 2018, Conlin 2021, RSI/RSIrs, VERDICT INNOVATE). **Verify every citation actually supports its claim.**

### D3. Submission roadmap (after Discussion, in order)
1. Conclusion (with the Discussion rework). 2. **Abstract** rework (≤250 words, passive, structured Purpose/Methods/Results/Conclusion, no formulae/citations). 3. Main figure legends should already be final — review **SI figure legends** once. 4. **Data Availability Statement** update + **GitHub repo cleanup**. 5. **Software DOI** (Zenodo — preferred). 6. Share full draft with **Stephan + Sandy** for review. 7. **Cover/motivational letter**. 8. Compute the **data/code SHA-1 hash** for the DAS. 9. **Submit.**

### D4. Open questions for co-authors (captured from the removed gated comments — resolve on the full-draft review)
- **Pixel-wise patient:** confirm it is in the 149-ROI cohort and the tumor is peripheral-zone (the "independent dataset" framing is already dropped in intro+methods accordingly). [Stephan]
- **Grading/histology interpretation** soundness (grade-dependent shifts; intermediate-bin reading). [Stephan]
- **GGG grouping:** we kept the cumulative **GGG ≥ 3** group because GGG = 3 alone is only $n=5$ (3:5, 4:2, 5:2); Stephan had suggested relabeling ≥3 as =3 — confirm cumulative is acceptable. [Stephan]
- **F13:** the two `4+3+5` cases coded GGG 5 vs ISUP 3. [Stephan]
...
**F10 — Bayesian CRLB resolves "factor-2000" gap (2026-05-26 diagnostic).** Stephan and Sandy flagged the magnitude of the unconstrained-CRLB vs NUTS posterior std gap (sometimes 2000×) as suspicious. Cause: comparison is apples-to-oranges. Diagnostic at SNR=303 (cohort median, NOT the hard-coded 150 in current Fig 8 script): unconstrained CRLB ranges 0.08–131; Bayesian CRLB (van Trees with HalfNormal prior λ=0.1) ranges 0.028–2.33 (2 orders of magnitude tighter); empirical NUTS std ranges 0.014–0.121 (another 5–50× tighter due to non-negativity + one-sided HalfNormal). Fix: re-build the figure with 3-bar comparison and reframe caption. **DECIDED 2026-06-22:** bar 2 named as the van-Trees bound under a *Gaussian approximation* of the half-normal prior (λ=0.1) — this is why NUTS legitimately sits below it (constraint gain), not a CRLB violation; the prior/constraint split is λ-dependent and non-orthogonal. Manuscript states this as our position; Sandy reviews the final draft rather than gating. Full write-up + diagnostic script: `notes/CRLB_NOTE_FOR_SANDY_v2_2026-06-21.md` (supersedes the original `.txt`).
...
**F12 — Detection positives are mpMRI radiological targets, not all biopsy-confirmed cancer (verified 2026-06-23 against `features.csv` + `utils/process_img_data.py` + `metadata.csv`).** The 40 "tumor" ROIs (detection positive class, n=81 PZ / n=68 TZ) are radiologist-drawn mpMRI targets. By histology: **29 biopsy-confirmed adenocarcinoma (GGG≥1)**, **7 benign (Gleason 0+0 → GGG 0)**, **1 HGPIN**, **3 with no separately recorded Gleason**. `features.csv` GGG value-counts among tumor ROIs: {0:7, 1:8, 2:12, 3:5, 4:2, 5:2, NaN:4}. The benign/HGPIN patients each contribute a tumor-target ROI used as a positive (the old Methods claim that they "contribute only normal-tissue ROIs" was FALSE). **Decision (Patrick 2026-06-23): keep all 40 targets as positives (no recompute); reframe Methods honestly (done) + flag in Discussion Limitations as radiological-target positives (ties to the existing circularity point — Stephan's 2026-05-22 point 3).** So detection = "radiologically suspicious lesion vs normal tissue," confirmed cancer in 29/40.

**F13 — Two `4+3+5` Gleason cases coded GGG 5 (verified 2026-06-23).** The loader takes GGG verbatim from `metadata.csv` `targets` column (`loaders.py:232`), which equals ISUP grade group EXCEPT for new03 (tumor_pz) and new45 (tumor_tz): Gleason `4+3+5` (tertiary pattern 5) is coded GGG **5**, whereas strict primary+secondary (4+3) is GGG **3**. Both are the only GGG=5 ROIs and both sit in the GGG≥3 bucket, so **Fig 5 (GGG1/2/≥3) and any binary low/high grading are unaffected**; only the *continuous*-GGG Spearman (F3, already explicitly tentative) would shift marginally if recoded 5→3. **Stephan to confirm** whether tertiary-pattern-5 upgrading to GGG 5 is intentional. Data left as-is pending his call.
...
> **⚠️ SUPERSEDED for numbering + section attribution (2026-06-22 eve).** The build-status notes below are still accurate per figure, but the figure→section mapping and numbering are now defined in §0 (Theory=Fig 1; Results=Fig 2–5 spectra/ROC/ADC-disc/GGG; Discussion=Fig 6–9 sensitivity/validation/uncertainty/pixel-wise). Ignore the old "Fig 8 = Fisher → SI" / pillar numbering here.
