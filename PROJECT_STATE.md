...
### Theory — `theory.tex` (3 subsections, RESTRUCTURED 2026-06-22 eve)
| St | Unit |
|----|------|
| 🔶 | Signal Model and Discretization — merged (Signal Model + Identifiability/Discretization); "can be modeled"; "conditioning can guide the grid"; heuristic = loose guide + physiology; refs Fig 1a/1c. Needs final pass. |
| 🔶 | Estimation — merged (MAP + Bayesian); two estimators from one non-negative model; ends with Fig 1b tractability (regularization + non-negativity gains). Needs final pass. |
| ☐ | ADC as a Spectral Functional (unchanged) |
