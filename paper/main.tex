...
\usepackage{pdfpages}  % required for the multi-page Supporting Information atlas (\includepdf)
...
%TC:ignore
\begin{titlepage}
{\noindent\LARGE\bf Why ADC works: identifiability and uncertainty in Bayesian decomposition of the prostate diffusivity spectrum}
\bigskip
...
ORCID       & \url{https://orcid.org/0000-0002-1329-4955} \\
\end{tabular}
...
\bigskip
\keywords{prostate cancer, diffusion-weighted MRI, apparent diffusion coefficient, Bayesian inference, uncertainty quantification, diffusivity spectrum}
%TC:break _main_
\pagebreak
...
% === Data Availability Statement (MRM: required in the main manuscript) ===
\section*{Data Availability Statement}
The source code, configuration files, and de-identified per-ROI signal decays
(with radiologist labels and Gleason Grade Groups) that support the findings of
this study are openly available at
\url{https://github.com/PaWeRe/spectra-estimation-dMRI}
% TODO(Patrick): add the exact commit SHA-1 used for this paper, and ideally a
% Zenodo (or equivalent) DOI, per MRM's reproducible-research guidelines;
% maintain the archive >= 5 years. State the IRB/consent conditions under which
% the human data are shared.
(commit \texttt{\todo{SHA-1}}). The repository packages the inference methods so
the full pipeline can be re-run on the released data or on new datasets.
%TC:endignore
