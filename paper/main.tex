...
The source code, configuration files, and the de-identified per-ROI signal decays and labels used in this study are openly available at \url{https://github.com/PaWeRe/spectra-estimation-dMRI} (archived at \todo{Zenodo DOI}; commit SHA-1 \todo{hash}). The repository packages the full inference, classification, and figure-generation pipeline so that the reported results can be reproduced and the methods applied to new datasets.
%TC:endignore
...
% === Figure Legends (MRM: caption text listed separately from the figures) ===
\pagebreak
