...
\begin{figure}[p]
\centering
% Tall 5x2 grid + long caption: cap the height so the figure and its caption
% both fit on one page (tune the 0.60 fraction on Overleaf if needed).
\includegraphics[width=\textwidth,height=0.60\textheight,keepaspectratio]{figures/fig8_v6.pdf}
\caption{%
...
recovered at a low (SNR $= 50$) and a high (SNR $= 1000$) noise level
(columns) that bracket the cohort median (303); the low end also reflects the
reduced per-voxel SNR of the pixel-wise analysis.
In each recovery panel the black marks are the truth, the orange box plots the
NUTS posterior for one noisy realization (box $=$ interquartile range, whiskers
...
in each panel title (all $\hat{R} \leq 1.02$).
...
a sampler artifact.
The bottom row shows joint noise inference: at each SNR the inferred noise
$\hat{\sigma}$ (NUTS posterior over 12 noisy realizations per ground truth,
drawn as box plots) is compared to the true $\sigma = 1/\mathrm{SNR}$ (dashed
line).
$\sigma$ is recovered accurately for the smooth spectra but is over-estimated
by $\sim$40--50\% on the $\delta$, where the model absorbs the unrepresentable
spike as excess noise (consistent with the coverage caveat discussed below).
}
