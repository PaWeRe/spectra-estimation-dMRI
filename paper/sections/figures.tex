...
% Supplementary: directional spectra, all-ROI atlas.
%
...
%
% ORDER = 3-block narrative (locked 2026-06-06, PROJECT_STATE 13.1), figures
% renumber 1-9 in reading order; semantic \label's auto-update every \ref:
%   Block A (method & what it can resolve):  1 spectra, 2 fisher, 3 validation
%   Block B (detection & why ADC works):     4 roc, 5 adc_discriminant, 6 sensitivity
%   Block C (beyond detection, exploratory): 7 uncertainty, 8 spectrum_ggg, 9 pixelwise
...
% --- Fig 2: Fisher-information analysis of the spectral discretization ---
% Generator: scripts/generate_fisher_figure.py -> fig_fisher_v2. 2-over-1
% layout (a,b top; c centered below); panel (b) three-bar comparison at the
% cohort-median SNR=303 with both improvement factors printed in-panel.
...
unconstrained Cram\'er--Rao lower bound (no prior), the Bayesian (van Trees)
CRLB incorporating the half-normal prior ($\lambda = 0.1$), and the empirical
NUTS posterior standard deviation. Two improvement factors are printed above
the bars: the prior gain (unconstrained $\to$ Bayesian, gray) and the
constraint gain (Bayesian $\to$ NUTS, orange). The prior tightens the bound by
up to two orders of magnitude, and the non-negativity constraint and one-sided
prior tighten it by a further $2$--$74\times$.
...
\begin{figure}[p]
\centering
\includegraphics[width=0.95\textwidth]{figures/fig8_v6.pdf}
\caption{%
Validation of the Bayesian inference on simulated ground truth across spectrum
shape and signal-to-noise ratio.
Four ground-truth spectra (normal-like, tumor-like, bimodal at $D = 0.25$ and
$3.0\;\umsmm$, and a concentrated $\delta$ at $D = 0.75\;\umsmm$) are each
recovered at SNR $= 50$ and $1000$ (columns) bracketing the cohort median (303).
In each recovery panel the black marks are the truth, the orange box plots the
NUTS posterior for one realization (box $=$ IQR, whiskers $=$ 5th--95th
percentile), and the green cross the tuned-MAP estimate; split-$\hat{R}$ is
in each title (all $\leq 1.02$).
Smooth spectra are recovered well at both SNRs, whereas the $\delta$ is
under-recovered and smeared into its correlated neighbors---the identifiability
limit of this grid, not a sampler artifact.
The bottom row shows joint noise inference: the inferred $\hat{\sigma}$ (box
plots over 12 realizations) versus the true $\sigma = 1/\mathrm{SNR}$ (dashed),
recovered accurately for smooth spectra but over-estimated by $\sim$40--50\% on
the $\delta$, where the model absorbs the unrepresentable spike as excess noise.
}
...
($r = -0.79$ PZ, $-0.88$ TZ), carried by the two outer bins.
...
% --- Fig 7: Uncertainty-aware classifier (posterior propagation) ---
\begin{figure}[p]
\centering
...
For each ROI the leave-one-out classifier (the NUTS-feature pipeline of
Figure~\ref{fig:roc}, $C = 1$) is applied to every posterior draw of the eight
fractions, yielding a distribution over $P(\mathrm{tumor})$. Per-ROI
predictions in the peripheral zone (top, $n = 81$) and transition zone (bottom,
$n = 68$), sorted by predicted probability: markers at the posterior mean
(circles normal, squares tumor), error bars the 90\% credible interval, pale
for correctly classified and saturated for misclassified ROIs.
The interval is $2.4\times$ wider for misclassified than correctly classified
ROIs in probability space, but this is largely the geometry of the logistic
link: in logit space the ratio falls to $1.3\times$ (bootstrap CI including one)
and the boundary correlation collapses, so the interval width does not add a
reliable error flag beyond the predicted probability in this cohort.
The few confidently-misclassified ROIs are almost all low-grade or ungraded
tumors whose spectra resemble normal tissue
(cf.\ Figure~\ref{fig:spectrum_ggg}).
This panel illustrates a capability of full posterior sampling (a calibrated
probability with a credible interval), not a validated biomarker; it is
exploratory.
}
\label{fig:uncertainty}
\end{figure}

% --- Fig 8: Diffusivity Spectrum by Gleason Grade Group ---
\begin{figure}[p]
\centering
...
\begin{figure}[p]
\centering
\includegraphics[width=0.95\textwidth]{figures/fig9_v2.pdf}
\caption{%
