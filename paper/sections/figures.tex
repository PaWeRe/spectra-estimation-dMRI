...
% --- Fig 1: Fisher-information analysis of the spectral discretization ---
% Generator: scripts/generate_fisher_figure.py -> fig_fisher_v2. Panel (b)
% rebuilt per PROJECT_STATE F10: three-bar unconstrained / van-Trees Bayesian /
% NUTS at the cohort-median SNR=303.
% TODO(Sandy): validate the van-Trees Bayesian-CRLB derivation in panel (b).
\begin{figure}[p]
\centering
\includegraphics[width=\textwidth]{figures/fig_fisher_v2.pdf}
\caption{%
...
% --- Fig 2: Cohort Diffusivity Spectra by Tissue and Zone ---
\begin{figure}[p]
\centering
...
% --- Fig 3: Method validation on simulated ground truth (spectrum x SNR battery) ---
...
% x 2 SNR recovery grid + per-SNR joint-noise sigma-calibration row. No Gibbs
% (deferred). Real pipeline: nuts.yaml + tuned-MAP ridge.yaml.
\begin{figure}[p]
\centering
% 5x3 grid (4 ground truths + a noise-sigma row) x 3 SNR columns;
% height-capped so it fits a float page.
\includegraphics[width=\textwidth,height=0.92\textheight,keepaspectratio]{figures/fig3_v7.pdf}
\caption{%
Validation of the spectral inference on simulated ground truth across spectrum
shape and noise level.
Four ground-truth spectra---normal-like and tumor-like (the cohort-mean NUTS
spectra), a uniform spectrum, and a concentrated $\delta$ at $D = 0.75\;\umsmm$%
---are each recovered at SNR $= 50$, $300$, and $1000$ (columns); SNR $300$ is
the cohort median (the operating point of the real data) and $50$ reflects the
reduced per-voxel SNR of the pixel-wise analysis.
In every panel the bars are the ground truth (black), the tuned-MAP estimate
($\lambda = 10^{-3}$, green), and the NUTS posterior mean (orange); error bars
are $\pm 1$ standard deviation across independent noise realizations, each fit
with the deployed NUTS configuration.
...
peak than MAP.
The bottom row shows noise recovery: the true $\sigma = 1/\mathrm{SNR}$ (dashed)
against the MAP residual estimate and the NUTS-inferred $\hat{\sigma}$, both
close to truth---the noise level is recoverable with or without a sampler.
The full six-ground-truth battery (adding inverse-tumor and bimodal spectra) is
shown in Figure~\ref{fig:recovery_si}.
}
\label{fig:validation}
\end{figure}

% --- Fig 4: Tumor-detection ROC; collapse onto the two outer compartments ---
\begin{figure}[p]
\centering
...
% --- Fig 5: ADC vs Spectral Discriminant ---
\begin{figure}[p]
\centering
...
% --- Fig 6: per-bin anatomy of the spectral discriminant ---
\begin{figure}[p]
\centering
...
% --- Fig 7: Diffusivity Spectrum by Gleason Grade Group (single-panel ladder) ---
\begin{figure}[p]
\centering
...
% --- Fig 8: Prediction confidence (posterior propagation through the classifier) ---
\begin{figure}[p]
\centering
% Long caption runs into the footer at full size: cap the height so the figure
% and its caption both fit on one page (tune the 0.64 fraction on Overleaf).
\includegraphics[width=\textwidth,height=0.64\textheight,keepaspectratio]{figures/fig6_v2.pdf}
\caption{%
