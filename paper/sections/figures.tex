...
% --- Fig 7: Fisher-information analysis of the spectral discretization ---
% Promoted from the Supplement 2026-06-05 (position 7: after Fig 6, before the
% validation battery). Keeps \label{fig:fisher} so the in-text references in
% theory/results/discussion resolve to this main figure.
...
% NUTS at the cohort-median SNR=303 (was hard-coded 150).
...
unconstrained Cram\'er--Rao lower bound (light gray, no prior), the Bayesian
(van Trees) CRLB incorporating the half-normal prior ($\lambda = 0.1$; dark
gray), and the empirical NUTS posterior standard deviation (orange).
The prior tightens the unconstrained bound by up to two orders of magnitude,
and the NUTS posterior is smaller still---the Bayesian-CRLB-to-NUTS ratio is,
from $D = 0.25$ to $20\;\umsmm$, $17$, $42$, $74$, $50$, $27$, $14$, $6$, and
$2\times$---because the non-negativity constraint and one-sided prior truncate
the feasible region.
