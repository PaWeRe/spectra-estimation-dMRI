...
Entries are leave-one-out cross-validated AUCs (95\% confidence interval) for
tumor-versus-normal classification, evaluated through an identical pipeline so
that single-feature and multi-feature models are directly comparable
(Figure~\ref{fig:roc}).
ADC (raw rank) ranks ROIs by the raw value; all other rows are
$L_2$-regularized logistic regression ($C = 1.0$) on standardized features.
