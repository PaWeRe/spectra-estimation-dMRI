...
and $1000$ (columns; SNR $300$, the round value nearest the cohort median of $303$).
...
three bin subsets: all eight fractions (orange solid), the restricted and free-water fractions
$\{D = 0.25,\,3.0\;\umsmm\}$ (orange dashed), and the five intermediate fractions
$\{D = 0.5$--$2.0\;\umsmm\}$ (orange dotted)---pass through the identical
...
restricted and free-water components, $D = 0.25\;\umsmm$ (restricted, turquoise) and $D = 3.0\;\umsmm$
(free water, brown); $D = 2.0\;\umsmm$ (dark yellow), $D = 1.5\;\umsmm$ (blue),
...
bottom).
All four panels are drawn on identical axes, but the discriminant is fit
separately per zone and estimator, so equal vertical positions in different
panels come from different fits and only the within-panel relationship is
meaningful; ADC ($x$-axis) is the same physical quantity throughout.
Tumor ROIs are red and normal ROIs blue; the black line is the
ordinary-least-squares (OLS) fit, and per-panel Pearson $r$ with a 95\% bootstrap
confidence interval is annotated (all $|r| > 0.95$).

\medskip\noindent\textbf{Figure~\ref{fig:sensitivity}.}
How much each diffusivity bin contributes to the spectral discriminant (bars)
and how strongly ADC responds to it (diamonds), by anatomical zone (PZ, TZ).
Both quantities are read on one axis: positive means ``pushes the readout toward
tumor.''
Bars are the tumor-versus-normal logistic-regression weights (NUTS features,
$C = 1.0$, the same fit as Figure~\ref{fig:adc_discriminant}), obtained after
rescaling every fraction to unit cohort standard deviation so that bins of very
different size can be compared directly, with 95\% bootstrap confidence
intervals; bar heights are then divided by the largest weight, so the tallest
bar is 1 by construction.
Black diamonds are the ADC sensitivity $-\partial\text{ADC}/\partial R_j$
evaluated at the average tumor spectrum, scaled the same way (the minus sign
orients it toward tumor, because tumor corresponds to low ADC).
...
distinct from the cohort-level weight stability shown by the bootstrap
intervals.
The annotated $r$ is the across-bin Pearson correlation between the weights and
the unflipped $\partial\text{ADC}/\partial\Rvec$, hence negative.
...
slice from a separate patient acquired outside the main 56-patient cohort.
...
(G,\,H)~Posterior standard deviation of the score (500 Monte Carlo draws) for
